...
Сборка css/js bitrix компонентов

```sh
gulp build # запустить сборку один раз
gulp watch # запустить сборку в режиме слежения за файлами
```
...
Мини-модуль [vueInvoker](local/distr/scripts/util/vueInvoker.js) 
...
Вывести на страницу элемент-плейсхолдер для компонента (все data атрибуты передаются вo vue):
